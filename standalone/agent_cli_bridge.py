"""
LangGraph ReAct agent for the claude_cli/copilot_cli backends only.

The ollama backend runs through standalone/agent.py's NOOA-based agent instead
(see that module's dispatch in analyse()). This module remains for claude_cli
and copilot_cli, which are subprocess-driven CLI tools bridged onto LangChain's
AIMessage/tool_calls shape by splunk.llm_backends — that bridge doesn't carry
over to NOOA's UnifiedLLM (litellm-based) client interface, so these two
backends stay on the old LangGraph ReAct loop until a NOOA-native bridge is
built for them (tracked as a follow-up task).
"""
...
def analyse_via_cli_bridge(findings: dict[str, Any]) -> tuple[str, list[str]]:
    """
    Run the ReAct agent over structured findings from detectors, using the
    claude_cli or copilot_cli backend (config.AGENT_BACKEND). Called from
    standalone/agent.py's analyse() dispatcher — not a direct entry point.
