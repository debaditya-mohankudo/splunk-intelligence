...
| `splunk__check_alerts` | Read unacknowledged alerts written by the standalone watcher (`standalone/watcher.py`), optionally filtered by `severity` (`critical`/`warning`/`info`) |
...
You have no self-scheduling mechanism, so continuous Splunk monitoring does not run in your loop — it runs as a separate standalone process (`uv run python -m standalone.watcher`, started outside this conversation) that polls Splunk on an interval and writes alerts to `splunk.db`.
...
If the user asks for continuous monitoring and the watcher isn't running, tell them to start it themselves (`uv run python -m standalone.watcher`, requires `SPLUNK_WATCH_SPL` set) — you cannot start a long-running background process on their behalf.
...
3. If `SPLUNK_KNOWN_INDEXES` is set, show the user that list alongside the SPLUNK_URL confirmation — reference context only, doesn't change what SPL gets run.
